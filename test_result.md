...
agent_communication:
  ...
  - agent: "main"
    message: "Fixed auth login bugs. Changes: 1) Fixed Python logging - added logging.basicConfig() so all WARNING/ERROR logs now appear in supervisor output (previously invisible). 2) Improved Google Auth with 3-attempt retry with exponential backoff (1s, 1.5s, 2.25s) to handle Emergent Auth session expiry. 3) Improved Salesforce password login - reduced timeout to 10s, added proper timeout exception handling. 4) Added PKCE store cleanup with 10-min TTL to prevent memory leaks (was accumulating entries indefinitely). 5) Added new /api/auth/diagnostics endpoint for debugging auth provider health. 6) Improved frontend error messages for Google, Salesforce, and credential login failures. 7) Re-registered apple@review.com in fresh DB environment. Please test all auth endpoints."
